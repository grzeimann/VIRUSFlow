...
| 2 | Bias+dark calibration, mask union | `tasks/science.py:117-168` (inline in `ReducedScienceAmplifierTask.run`) | AMP | oriented image/variance, `master_bias`, `master_dark`, `master_ldls`, raw dark EXPTIME | arithmetic only, no algorithm function | `ReducedAmplifierState` (image, variance, mask, parent_ids, summaries) | none (scratch) | `ReducedAmplifierState` | Physical-CCD assembly (op 3) | `RuntimeError` if bias/dark missing (aborts this amplifier via caller's try/except) | not in the requested list explicitly; closest is scattered-light precursor |
